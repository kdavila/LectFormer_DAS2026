...
from LM_Tools.data.connected_component import ConnectedComponent
from LM_Tools.util.time_helper import TimeHelper
from LectureMath.preprocessing.content.labeler import Labeler
...
class KeyFrameAnnotation:
    def __init__(self, database, lecture, frame_idx, frame_time, frame_objects, raw_image):
        ...
        # Note that this used to be "object_mask"
        self.background_mask = None

        if raw_image is not None:
            self.update_binary_image(False)
            # this is now done here
            self.update_background_mask()
    ...
    def check_cc_overlaps_background(self, cc):
        ...
        if cc.max_x < 0 or cc.min_x >= self.background_mask.shape[1] or cc.max_y < 0 or cc.min_y >= self.background_mask.shape[0]:
            # completely out of boundaries and assumed as part of the background ...
            return True

        # use pre-computed background mask
        # first, find the cut for the given cc
        mask_cut = self.background_mask[cc.min_y:cc.max_y + 1, cc.min_x:cc.max_x + 1]
    ...
    def update_object_mask(self):
        ...
        cv2.imshow("objects", tempo_mask)
        cv2.waitKey()
        """
        raise Exception("This function has been deprecated")

    def update_background_mask(self, mask_expansion=5):
        if self.binary_cc is None or len(self.binary_cc) == 0:
            # nothing to update ...
            return

        # compute and store all CC's corners in a list ...
        all_corners = []
        for cc in self.binary_cc:
            all_corners.append((cc.min_x - mask_expansion, cc.min_y - mask_expansion)) # top-left
            all_corners.append((cc.max_x + mask_expansion, cc.min_y - mask_expansion))  # top-right
            all_corners.append((cc.min_x - mask_expansion, cc.max_y + mask_expansion))  # bottom-left
            all_corners.append((cc.max_x + mask_expansion, cc.max_y + mask_expansion))  # bottom-right

        hull_mask = np.zeros((self.binary_image.shape[0], self.binary_image.shape[1]), dtype=np.uint8)

        # use the convex hull of the CCs ...
        try:
            # try to compute convex hull
            hull = ConvexHull(all_corners)
            polygon = []
            for vertex in hull.vertices:
                polygon.append(all_corners[vertex])

            cv2.fillPoly(hull_mask, [np.array(polygon)], (255,))
        except QhullError as e:
            # convex-hull algorithm
            # it can fail if all points have the same coordinate, simply use the whole area as the mask
            hull_mask[:, :] = 255

        # invert, so that anything outside of the hull is considered background ...
        self.background_mask = 255 - hull_mask
    ...
    def update_combined_image(self):
        ...
        # The object mask has been deprecated
