...
from LectureMath.annotation.data.keyframe_annotation import KeyFrameAnnotation
from LectureMath.data.meta_data_DB import MetaDataDB
from LectureMath.evaluation.evaluator import Evaluator
from LectureMath.evaluation.eval_parameters import EvalParameters
...
def main():
    if len(sys.argv) < 4:
        ...
        print("\tdataset:\tName of the dataset")
        return
    ...
    # load the database
    try:
        database = MetaDataDB.from_file(config.get_str("LectureNet.LectureDataset.DataBasePath"))
    except:
        print("Invalid database file")
        return

    if database.get_dataset(dataset) is None:
    ...
    output_dir = config.get_str("LectureNet.General.OutputPath")
    DEVICE = config.get("LectureNet.General.Device", "cuda:0")
    binary_save_dir = config.get_str("LectureNet.Evaluation.SaveBinary.Path", ".")

    start_loading = time.time()

    print("... loading model ...")
    lecture_net = LectFormer.CreateFromConfig(config, 3)
    lecture_net.load_state_dict(torch.load(model_filename, map_location="cpu"))
    ...
    for idx, bin_kf in enumerate(binarized_keyframes):
        ...
        if config.get("LectureNet.Evaluation.SaveBinary.Enabled", True):
            binary_dir = output_dir + "/" + binary_save_dir + "/" + dataset
